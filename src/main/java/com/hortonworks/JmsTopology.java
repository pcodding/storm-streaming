...
public class JmsTopology {
	...
	public static void main(String[] args) throws Exception {
		...
		JmsProvider jmsTopicProvider = new SpringJmsProvider(
				"jms-activemq.xml", "jmsConnectionFactory", "notificationTopic");
		...
		builder.setBolt(
				INTERMEDIATE_BOLT,
				new GenericBolt("INTERMEDIATE_BOLT", true, true, new Fields(
						"json")), 3).shuffleGrouping(JMS_QUEUE_SPOUT);
		...
		builder.setBolt(FINAL_BOLT, new GenericBolt("FINAL_BOLT", true, true),
				3).shuffleGrouping(INTERMEDIATE_BOLT);

		// bolt that subscribes to the intermediate bolt, and publishes to a JMS
		// Topic
		...
		builder.setBolt(JMS_TOPIC_BOLT, jmsBolt).shuffleGrouping(
				INTERMEDIATE_BOLT);
		...
		builder.setBolt(ANOTHER_BOLT,
				new GenericBolt("ANOTHER_BOLT", true, true), 1)
				.shuffleGrouping(JMS_TOPIC_SPOUT);
		...
		if (args.length > 0) {
			...
			cluster.submitTopology("storm-jms-example", conf,
					builder.createTopology());
			Utils.sleep(60000);
